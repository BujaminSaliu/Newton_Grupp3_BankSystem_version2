...
public class BorderPaneTestController implements Initializable
{
    ...
    {
        String firstNameAndLastName = nameTextField.getText();
        String[] arrCustomerName = firstNameAndLastName.split(" ");
        ...
        } else if (arrCustomerName.length < 2)
        {
            returnMessageToOperator.setText("Ange kundens först och efternamn.");
        ...
        } else if (!arrCustomerName[0].matches("^[A-zåäöÅÄÖ-]+$") || !arrCustomerName[1].matches("^[A-zåäöÅÄÖ-]+$"))
        {
    ...
    private void changeCustumerNameButton(ActionEvent event) throws Exception
    {
        ...
        try
        {
            ...
            if (name.isEmpty())
            {
                returnMessageToOperator.setText("Välj kund och ange nytt först och efternamn.");
            } else if (arrName.length < 2)
            {
                returnMessageToOperator.setText("Ange nytt först och efternamn.");
            } else if (!arrName[0].matches("^[A-zåäöÅÄÖ-]+$") || !arrName[1].matches("^[A-zåäöÅÄÖ-]+$"))
            {
                returnMessageToOperator.setText("Namn får endast bestå av bokstäver!");
            } else
            {
                ...
                for (int i = 0; i < bankLogic.getAllAccount(pNr).size(); i++)
                {
                    if (bankLogic.getAllAccount(pNr).get(i).toString2().equals(selectedAccountString))
                    {
                        ...
                        if (bankLogic.getAllAccount(pNr).get(i).getAccountType().matches("Credit"))
                        {
                            ...
                            } else if (bankLogic.withdraw(accountID1, amount))
                            {
