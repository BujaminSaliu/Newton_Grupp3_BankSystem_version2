    ...
    public final void getOnMouseClickedCustListView()
    {
        ...
        for (int i = 0; i < bankLogic.allCustomersArrayList.size(); i++)
        {
            if (bankLogic.allCustomersArrayList.get(i).toString2().equals(selectedCustomerString))
            {
                ...
                nameDisplayLabel.setText(bankLogic.allCustomersArrayList.get(i).getCustomerName());
                pNrDisplayLabel.setText(Long.toString(bankLogic.allCustomersArrayList.get(i).getPersonalNumber()));
  ...
  @FXML
    private void findCustumerButton(ActionEvent event) throws Exception
    {
        if(searchTextField.getText().isEmpty())
        {
            ...
            obListAllCustumers.addAll(bankLogic.getCustomers());
            custumersListView.setItems(obListAllCustumers);
            returnMessageToOperator.setText("Ingen kund hittad!");
        }
        else
        {
        ...
        for (int i = 0; i < bankLogic.allCustomersArrayList.size(); i++)
        {
            if (bankLogic.allCustomersArrayList.get(i).toString2().contains(searchTextField.getText()))

            {

                //custumersListView.getItems().clear();
                
                obListFoundCustumers.add(bankLogic.allCustomersArrayList.get(i).toString2());
